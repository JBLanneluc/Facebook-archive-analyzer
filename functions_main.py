...
# Renvoit 2 listes pour une discussion precise : celle des messages recus et celle des messages envoyes avec
# l'expéditeur
...
# Renvoie la liste des fichiers de discussion à 2 et la liste des fichiers de discussions de groupe (chemin complet
# des fichiers)
...
# Renvoie uniquement les discussions à 2, pas les discussions de groupes, sous la forme [[timestamp, content, sender,
# receiver],...]
...
def all_directories_timestamp_messages_parser(dir_path):
    ...
    for file in file_list:
        ...
        received_timestamp_list = received_timestamp_list + file_received  # Liste des timestamps des messages reçus
        # et de l'expéditeur : [[timestamp1, expéditeur1], [timestamp2, expéditeur2]]...
...
def normalize_dataframe(messages_list):
    ...
    normalized_dataframe = pd.DataFrame(
        {'day': day, 'month': month, 'year': year, 'hour': hour, 'minute': minute, 'second': second,
         'month_number': month_number, 'weekday': weekday, 'Date': pd.to_datetime(Date), 'sender': sender,
         'receiver': receiver})
    to_drop.append(index)
...
def test_dataframe():
    ...
    normalized_dataframe = pd.DataFrame(
        {'day': day, 'month': month, 'year': year, 'hour': hour, 'minute': minute, 'second': second,
         'month_number': month_number, 'weekday': weekday})

    return normalized_dataframe


def convert_month_number_to_name(month_number):
    month_name = {1: "Janvier", 2: "Fevrier", 3: "Mars", 4: "Avril", 5: "Mai", 6: "Juin", 7: "Juillet", 8: "Aout",
...
# renvoie un dataframe contenant untiquement les donnees datees entre les 2 dates d'entrees (donnees au format
# "dd/MM/YYYY")
...
messages_sent, messages_received, dir_dictionary, username, raw_username = all_directories_timestamp_messages_parser(
    dir_path)
print(username)
